...
void recolectarInstrucciones(char* pathDelArchivoDeInstrucciones){
	...
	contenidoDelArchivo = calloc(ftell(archivoDeInstrucciones) + 1,sizeof(char));
		...
		for (i=0;feof(archivoDeInstrucciones) == 0;i++) {
			caracter = fgetc(archivoDeInstrucciones);
			contenidoDelArchivo[i] = caracter;

			if(contenidoDelArchivo[i] == ';'){
				//significa que hay una instruccion completa con sus parametros
				char* contenidoAux = calloc(i,sizeof(char));
				contenidoAux = string_substring_until(contenidoDelArchivo,i);
				contenidoAux[i] = '\0';
...
void ejecutarMProc(char* pathDelArchivoDeInstrucciones){
	...
	for(;!feof(archivoDeInstrucciones) && !ferror(archivoDeInstrucciones);){
		...
		long unsigned tamDeLosDatosRecibidos = RecibirDatos(socket_Memoria,datosRecibidos);
